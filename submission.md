...
I reproduced this through the existing streak test. The test simulated a user listening on Saturday and then again on Sunday. The expected streak was 2 because those are consecutive calendar days, but the actual streak stayed at 1.

### How I found the root cause

I opened `services/streak_service.py` and followed the streak update logic in `update_listening_streak()`. The function calculated `days_since_last` correctly, but the conditional for consecutive days also checked `today.weekday() != 6`. That made me confident this was the root cause because Python uses `6` for Sunday, matching the reported Sunday-only reset.

### The root cause

The streak logic incorrectly excluded Sundays from the consecutive-day increment path. Even when `days_since_last == 1`, the condition `today.weekday() != 6` prevented the streak from incrementing on Sunday. The code then fell into the `else` block and reset the streak to 1.

### My fix and side-effect check

I removed the Sunday exclusion and allowed any `days_since_last == 1` case to increment the streak. After the fix, I ran `pytest tests/test_streaks.py` to confirm Saturday-to-Sunday increments correctly and that the other streak behaviors still passed.
